  ...
  useEffect(() => {
    ...
    const handleAppleCollision = (newSnake) => {
      ...
      if (snakeHead.x === apple.x && snakeHead.y === apple.y) {
        setScore(prevScore => prevScore + 1); // Fixed score increment

        setApple({
          x: Math.floor((Math.random() * (canvas.width - 14)) / SNAKE_SPEED) * SNAKE_SPEED,
          y: Math.floor((Math.random() * (canvas.height - 14)) / SNAKE_SPEED) * SNAKE_SPEED,
        });
